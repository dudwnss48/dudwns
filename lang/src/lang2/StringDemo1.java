...
public class StringDemo1 {
	...
	public static void main(String[] args) {
		...
		// String substring(int begin) : 지정된 위치부터 끝까지 포함된 새로운 문자열을 반환한다.
		...
		System.out.println("3번째 부터 9번째앞까지를 포함하는 문자열: " + s2);
		
		// String toUpperCase() : 문자열에 대한 새로운 대문자로 변환된 문자열을 반환한다.
		// String toUpperCase() : 문자열에 대한 새로운 소문자로 변환된 문자열을 반환한다.
		...
		System.out.println("대문자로 구성됭 문자열: " + s3);
		System.out.println("소문자로 구성된 문자열: " + s4);
		
		// String replace(CharSequence old, CharSequence nw) : 문자열에서 지정된 문자열(odl)을 새문자열(nw)로 변환한 새로운 문자열을 반환한다.
		// String replaceAll(String regexp, String replacement) : 문자열에서 지정된 정규표현식과 일치하는 문자열을 찾아서 새 문자열로 변환한 새로운 문자열을 반환한다.
		String str5 = "중앙HTA학원과 새중앙HTA학원은 별도의 법인이다.";
		String s5 = str5.replace("중앙", "현대");
		System.out.println("원본 문자열: " + str5);
		System.out.println("중앙이 현대로 바뀐 새 문자열: " + s5);
		...
		String str7 = "S0001,정자공학과, 회로이론실습, 홍길동, 3";
		...
		System.out.println("문자열에 '주사'를 포함하고 잇나? " + b3);
		...
		System.out.println("ms워드 파일인가?? " + b7);
